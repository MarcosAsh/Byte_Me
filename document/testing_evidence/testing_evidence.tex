...
\begin{itemize}
    ...
    \item Tested edge cases, specifically preventing orders when stock is insufficient.
...
\noindent \textbf{Justification:}
Trust is key to a successful marketplace. If there is a problem with an order, we need a reliable way to track and fix it. These tests ensure that every complaint is recorded and that there is a clear "paper trail" showing when and how a problem was solved.
...
% --- Section 8: Manual Testing Video Evidence ---
\section{Manual User Acceptance Testing (Video Evidence)}

\textbf{Overview:} 
To complement our automated suite, we performed a  end-to-end manual walkthrough. This recording demonstrates the live synchronization between the Seller and Organization (Consumer) roles, confirming the full cycle of a food bundle from creation to collection.

\noindent \textbf{Justification:}
This video acts as a comprehensive "walkthrough" of the most critical user path for our application. By recording this manual session, we prove that the integration between the frontend UI and the Spring Boot backend is seamless and that the database correctly handles transactional changes (like stock reservation) in a live environment.

\subsection*{Walkthrough Key Actions \& Timestamps}
\begin{table}[H]
    \centering
    \small
    ...
    \begin{tabular}{|l|p{8cm}|c|}
        \hline
        \rowcolor{gray!10} \textbf{Phase} & \textbf{Action Performed} & \textbf{Timestamp} \\ \hline
        ...
        & \textbf{Bundle Creation:} Posting new food surplus for rescue & [01:00] \\ \hline
        \textbf{Organization} & Listing Discovery: Viewing & [02:40] \\ \cline{2-3}
        \textbf{Workflow} & \textbf{Reservation 1:} Booking a bundle and generating code & [02:50] \\ \cline{2-3}
        & Active Claims: Viewing reservations tab & [03:00] \\ \cline{2-3}
        & \textbf{Cancellation:} Testing the order reversal logic & [03:40] \\ \cline{2-3}
        & \textbf{Reservation 2:} Re-booking and new code generation & [04:00] \\ \hline
        ...
        \textbf{Persistence} & \textbf{Persistence Check:} Logging out and back in successfully & [05:00] \\ \hline
        \textbf{Impact} & \textbf{Environmental Impact:} Viewing CO2 and forecast stats & [05:18] \\ \cline{2-3}
        & \textbf{Achievements:} Reviewing "First Rescue" badge & [05:35] \\ \hline
    \end{tabular}
    \caption{Manual Testing Time-Log: Seller and Consumer Integration}
\end{table}
...
\begin{figure}[H]
    ...
    \caption{Manual Testing Demonstration: Live Application Walkthrough}
\end{figure}
